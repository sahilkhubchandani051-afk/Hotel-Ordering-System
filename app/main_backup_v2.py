...
async def debug_logging_middleware(request: Request, call_next):
    ...
    try:
        # Log to file
        with open("C:/Users/usery/.gemini/antigravity/scratch/hotel_app/debug_middleware.txt", "a") as f:
            f.write(f"REQ: {request.method} {request.url}\n")
            
        print(f"DEBUG_MW: {request.method} {request.url}", file=sys.stderr)
        ...
        import traceback
        print(f"DEBUG_MW EXCEPTION: {e}", file=sys.stderr)
        with open("C:/Users/usery/.gemini/antigravity/scratch/hotel_app/debug_middleware.txt", "a") as f:
...
async def customer_quotations(request: Request):
    ...
    # DEBUG: Log quotation visibility data
    print(f"[DEBUG] Customer {customer_email} - Found {len(prs_rows)} quotations")
    for pr in prs_rows:
        print(f"[DEBUG] Quotation ID: {pr['id']}, Status: {pr['status']}, PO ID: {pr['po_id']}")
...
async def create_invoice_submit(request: Request, po_id: int):
    ...
    try:
        ...
        cursor.execute('''
            UPDATE purchase_orders 
            SET delivery_status = 'INVOICED', delivered_at = ?, items_json = ?
            WHERE id = ?
        ''', (created_at, json.dumps(updated_items), po_id))
