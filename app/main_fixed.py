...
from app.utils import number_to_words, calculate_next_delivery_date, is_quotation_expired
try:
...
async def translate_text(request: Request):
    try:
        ...
        if GoogleTranslator:
            try:
                # Use deep-translator (synchronous, but fast enough for short text)
        ...
        return JSONResponse({'translatedText': text if 'text' in locals() else ''})

@app.post("/api/parse_with_grok")
async def parse_with_grok(request: Request):
    """
    Optional Grok AI enhancement for Quick Add parsing.
    Returns structured item suggestions or empty array if unavailable.
    """
    try:
        data = await request.json()
        ...
        if not grok_api_key or not requests:
            return JSONResponse({'suggestions': []})
        
        # Prepare Grok prompt - INTENT EXTRACTION ENGINE
        prompt = f"""You are an INTENT EXTRACTION ENGINE for a live B2B ordering application.

IMPORTANT CONSTRAINTS:
- DO NOT change or suggest any UI changes
- DO NOT change existing PR, Cart, Quotation, PO, or Invoice flows
- DO NOT break existing English Quick Add behavior
- Your output will be consumed by an existing parser and UI
- The UI already has a review-and-confirm step

USER INPUT CHARACTERISTICS:
- May be English, Marathi, Hindi, or mixed languages
- May be spoken or typed
- May have any word order
- May omit connector words
- May omit quantity or unit
- May contain extra filler words

YOUR ONLY TASK:
Extract structured purchase intent from the user input.
...
NORMALIZATION RULES:
- Product names should match closest English product name if possible
- Units must be normalized to one of:
  kg, gram, packet, pcs, liter
- If quantity is missing, assume qty = 1
- If unit is missing, set unit = null

OUTPUT FORMAT (EXACT):
[
  {{ "item": "<product_name>", "qty": <number>, "unit": "<unit_or_null>" }}
]

If you are unsure about a product match:
- Still include it with best guess
...
{text}"""
        ...
        try:
            # Detect if it's a Groq key (starts with gsk_) or x.ai key
            ...
            model_name = "llama-3.3-70b-versatile" if is_groq else "grok-beta"
            ...
            if response.status_code != 200:
                print(f"[WARN] AI API returned status {response.status_code}: {response.text}")
                return JSONResponse({'suggestions': []})
            ...
            if content.startswith('```'):
                ...
                if match:
                    ...
                    # Fallback: remove backtick lines
                    lines = content.split('\n')
            ...
            try:
                ...
                print(f"[ERR] AI JSON Parse Error: {je}")
                # Try simple regex extraction if json.loads fails
            ...
            if not isinstance(suggestions, list):
                print(f"[WARN] AI returned non-list: {type(suggestions)}")
                suggestions = []
            
            # Ensure each item has required fields and map to frontend keys
            validated = []
            for item in suggestions:
                if isinstance(item, dict):
                    # Handle both "item" (new) and "product_name" (old/fallback)
                    p_name = item.get('item') or item.get('product_name')
                    if p_name:
                        validated.append({
                            'product_name': p_name,
                            'qty': float(item.get('qty', 1)),
                            'unit': item.get('unit') or 'pcs' # Default to pcs if null/missing
                        })
            
            return JSONResponse({'suggestions': validated})
            
        except requests.exceptions.Timeout:
            print("[WARN] Grok API timeout")
            return JSONResponse({'suggestions': []})
        except requests.exceptions.RequestException as e:
            print(f"[WARN] Grok API request failed: {e}")
            return JSONResponse({'suggestions': []})
        except json.JSONDecodeError as e:
            print(f"[WARN] Grok API returned invalid JSON: {e}")
            return JSONResponse({'suggestions': []})
        except Exception as e:
            print(f"[WARN] Grok API unexpected error: {e}")
            return JSONResponse({'suggestions': []})
            
    except Exception as e:
        print(f"[ERR] Parse with Grok error: {e}")
        return JSONResponse({'suggestions': []})
    ...
    try:
        ...
        c.execute(f'''
            CREATE TABLE IF NOT EXISTS products (
                ...
                image_url TEXT,
                created_at TEXT,
                reorder_level_shop INTEGER DEFAULT 5,
                reorder_level_godown INTEGER DEFAULT 10,
        ...
        c.execute(f'''
            CREATE TABLE IF NOT EXISTS purchase_orders (
                ...
                delivery_status TEXT DEFAULT 'OPEN',
                delivery_stage VARCHAR(50) DEFAULT 'ORDER_PLACED',
                tracking_status TEXT DEFAULT 'PO_CREATED',
...
def get_current_delivery_stage(po: dict, current_date: date = None) -> tuple:
    ...
    if expected_date and current_date >= expected_date:
    ...
    elif expected_date and current_date >= expected_date - timedelta(days=1):
        ...
        if current_date >= po_date + timedelta(days=1):
            return ('PACKAGED', (po_date + timedelta(days=1)).strftime('%Y-%m-%d 00:00'))
...
def update_delivery_timestamps(po_id: int):
    ...
    try:
        ...
        elif stage == 'PACKAGED' and not po.get('packaged_at'):
...
async def login(request: Request, email: str = Form(...), password: str = Form(...)):
    ...
    if supabase:
        try:
            ...
            # Supabase login failed (e.g., user not found or wrong password)
            # Fall through to local database check for development/test accounts
            print(f"Supabase login failed, trying local fallback for {email}: {e}")
    
    # 3. Fallback to local database authentication
    conn = get_db_connection()
    user = conn.execute("SELECT * FROM customers WHERE email = ?", (email,)).fetchone()
    conn.close()
...
async def toggle_favorite(data: ToggleFavoriteRequest, request: Request):
    require_admin(request)
    conn = get_db_connection()
    try:
        # User defined SQL with RETURNING
        cursor = conn.execute(
            """
            UPDATE customers
        ...
        if row is None:
             raise HTTPException(status_code=404, detail="Customer not found or no change made")
             
        return {"is_favorite": row[0]}
    except Exception as e:
        conn.close()
        raise e
    finally:
...
async def product_form(request: Request, id: Optional[int] = None):
    require_admin(request)
    conn = get_db_connection()
    categories = conn.execute("SELECT * FROM categories ORDER BY name").fetchall()
...
async def add_product(
    ...
    if not name_marathi and name:
        ...
        name_marathi = translate_text(name, "mr")
    ...
    conn.execute("INSERT INTO products (name, name_marathi, unit, rate, favorite_price, category_id, image_path, image_url, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                 (name, name_marathi, unit, rate, favorite_price, cat_id, filename, image_url, created_at))
    conn.commit()
    conn.close()
    
    return RedirectResponse(url="/admin/products", status_code=303)

@app.get("/admin/products/sample_template")
async def download_sample_template(request: Request, format: str = "csv"):
    """Generates a sample template for bulk product import"""
    require_admin(request)
    
    headers = ["name_en", "name_mr", "category", "unit", "base_rate", "favorite_price", "image_url"]
    sample_data = [
        ["Fresh Tomato", "ताजा टोमॅटो", "Vegetables", "kg", "45.0", "42.0", ""],
        ["Amul Milk 1L", "अमूल दूध १ ली", "Dairy", "ltr", "66.0", "", "https://example.com/milk.jpg"]
    ]
    ...
    if format == "csv":
        ...
        response.headers["Content-Disposition"] = "attachment; filename=product_import_sample.csv"
        return response
    else:
        # Excel format (XLSX)
        if Workbook is None:
            return JSONResponse(status_code=500, content={"error": "Excel support (openpyxl) not installed"})
        ...
        response = Response(content=out.read(), media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        response.headers["Content-Disposition"] = "attachment; filename=product_import_sample.xlsx"
        return response

@app.post("/admin/products/bulk_import")
async def bulk_import_products(request: Request, file: UploadFile = File(...)):
    """Handles bulk product import from CSV or XLSX"""
    require_admin(request)
    ...
    try:
        if filename.endswith(".csv"):
            ...
            if Workbook is None:
                return JSONResponse(status_code=500, content={"error": "Excel support not installed"})
            ...
            for row_cells in ws.iter_rows(min_row=2, values_only=True):
                ...
                products_to_add.append(dict(zip(headers, row_cells)))
        else:
            return JSONResponse(status_code=400, content={"error": "Invalid file format. Use CSV or XLSX."})
    ...
    categories_row = conn.execute("SELECT id, name FROM categories").fetchall()
    category_map = {c['name'].lower(): c['id'] for c in categories_row}
    
    # Pre-fetch existing products to prevent duplicates (Name + Unit)
    existing_products = conn.execute("SELECT name, unit FROM products").fetchall()
    existing_set = {(p['name'].lower(), p['unit'].lower()) for p in existing_products}
    
    created_at = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    for i, row in enumerate(products_to_add, start=2):
        try:
            name_en = str(row.get("name_en", "")).strip()
            unit = str(row.get("unit", "")).strip()
            rate = row.get("base_rate")
            
            if not name_en or not unit or rate is None:
                failed += 1
                errors.append(f"Row {i}: Missing required fields (name_en, unit, base_rate)")
                continue
            ...
            try:
                ...
                failed += 1
                errors.append(f"Row {i}: Invalid base_rate")
                continue

            if (name_en.lower(), unit.lower()) in existing_set:
                skipped += 1
                continue

            # Map category
            cat_name = str(row.get("category", "")).strip()
            cat_id = category_map.get(cat_name.lower()) if cat_name else None
            
            # Optional fields
            name_mr = str(row.get("name_mr", "")).strip()
            if not name_mr:
                from app.utils import translate_text
                name_mr = translate_text(name_en, "mr")
                
            fav_price = row.get("favorite_price")
            try:
                fav_price = float(fav_price) if fav_price is not None and str(fav_price).strip() else None
            except:
                fav_price = None
            ...
            conn.execute("""
                ...
                (name, name_marathi, unit, rate, favorite_price, category_id, image_url, created_at) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (name_en, name_mr, unit, rate, fav_price, cat_id, img_url, created_at))
            
            imported += 1
            existing_set.add((name_en.lower(), unit.lower())) # Add to set to prevent duplicates within same file
    ...
    return JSONResponse(content={
        ...
        "skipped": skipped,
        "failed": failed,
        "errors": errors[:10] # Return first 10 errors to avoid overwhelming UI
    })
...
async def edit_product(
    ...
    # Handle both Form and JSON for backward compatibility
    if name is None:
        try:
            data = await request.json()
            name = data.get('name')
            ...
            image_url = data.get('image_url', '')
             ...
             return JSONResponse(status_code=400, content={"error": "Invalid form or JSON data"})

    if not name or not unit or rate < 0:
        return JSONResponse(status_code=400, content={"error": "Missing name, unit or invalid rate"})
    ...
    if filename:
        conn.execute("""
            UPDATE products 
            SET name=?, name_marathi=?, unit=?, rate=?, favorite_price=?, category_id=?, image_path=?, image_url=? 
            WHERE id=?
        """, (name, name_marathi, unit, rate, favorite_price, cat_id, filename, image_url, product_id))
    else:
        conn.execute("""
            UPDATE products 
            SET name=?, name_marathi=?, unit=?, rate=?, favorite_price=?, category_id=?, image_url=? 
            WHERE id=?
    ...
    # If it was a form submit, redirect. If AJAX, return JSON.
...
async def admin_quotation(request: Request, pr_id: int):
    ...
    # Hydrate items with current product names, SHOP stock, and reorder levels
    inventory_service = InventoryService(conn)
    hydrated_items = []
    for item in items:
        prod = conn.execute("SELECT * FROM products WHERE id = ?", (item['product_id'],)).fetchone()
        if prod:
            shop_stock = inventory_service.get_product_shop_stock(item['product_id'])
            hydrated_items.append({
                ...
                "reorder_level_shop": prod['reorder_level_shop'] or 0
            })
...
async def customer_statement(
    ...
    # Get all POs for this customer that have FINAL invoices (invoice_no exists)
    # This ensures statement only shows billable amounts
    query = '''
    ...
    for po_row in all_pos:
        ...
        inv_amt = float(po.get('total_amount') or 0.0)
        ...
        entry = {
            "id": po['id'],
            "display_id": po.get('invoice_no'),
            "pr_id": po.get('pr_id'),
            "date": date_str,
            "category": po.get('customer_category'),
...
@app.get("/customer/statement/export")
async def export_customer_statement(
    ...
    # Get all POs for this customer that have FINAL invoices (invoice_no exists)
    query = '''
        SELECT po.*, c.category as customer_category, c.name as customer_real_name, i.invoice_no
        FROM purchase_orders po
        JOIN customers c ON po.customer_email = c.email
        LEFT JOIN invoices i ON i.po_id = po.id
        WHERE po.customer_email = ? 
    ...
    for po_row in all_pos:
        po = dict(po_row)
        customer_name = po.get('customer_real_name', customer_name)
    ...
    ws['A4'] = "Outstanding Balance"
    ws['B4'] = running_balance
    ...
    # Table Header
    headers = ["Date", "Invoice No", "Type", "Opening Balance", "Invoice Amount", "Amount Received", "Closing Balance"]
    ...
    # Table Data
    for idx, entry in enumerate(ledger_entries, start_row + 1):
        ws.cell(row=idx, column=1).value = entry['date']
        ws.cell(row=idx, column=2).value = entry['display_id']
        ws.cell(row=idx, column=3).value = "Invoice" # Based on logic, these are all invoiced POs
        
        # Numbers with formatting
        col_indices = [4, 5, 6, 7]
        col_values = [entry['opening_bal'], entry['inv_amt'], entry['amt_rec'], entry['closing_bal']]
    ...
    filename = f"Statement_{customer_name.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.xlsx"
...
async def admin_pr_view(request: Request, pr_id: int):
    ...
    query = '''
        SELECT pr.*, c.name as customer_name, c.is_favorite
        FROM purchase_requisitions pr
        JOIN customers c ON pr.customer_email = c.email
        WHERE pr.id = ?
    '''
    ...
    pr = dict(pr_row)
    items = json.loads(pr["items_json"])
    is_favorite_customer = bool(pr.get('is_favorite'))
    ...
    for item in items:
        ...
        prod = conn.execute("SELECT name, name_marathi, unit, rate, favorite_price FROM products WHERE id = ?", (item['product_id'],)).fetchone()
        if prod:
            item_name = prod['name']
            ...
            if quoted_rate is None:
                if is_favorite_customer and prod['favorite_price'] and prod['favorite_price'] > 0:
                     quoted_rate = prod['favorite_price'] # Apply Favorite Price
                else:
                     quoted_rate = prod['rate'] # Standard Rate
        else:
...
async def report_ledger(
    ...
    if customer_email:
        ...
        # Get all POs for this customer that have FINAL invoices (invoice_no exists)
        # This ensures ledger only shows billable amounts
        query = '''
            SELECT po.*, c.category as customer_category, i.invoice_no
            FROM purchase_orders po
            JOIN customers c ON po.customer_email = c.email
        ...
        for po_row in all_pos:
            ...
            entry = {
                "id": po['id'],
                "display_id": po.get('display_id') or f"PO-{po['id']}",
                "pr_id": po.get('pr_id'),
                "date": date_str,
                "category": po.get('customer_category'),
                "opening_bal": opening_bal,
...
async def update_ledger_payment(request: Request):
    ...
    if po_id:
        ...
        if po:
            ...
            all_pos = conn.execute("SELECT total_amount, amount_received, invoice_receipt, delivery_status FROM purchase_orders WHERE customer_email = ? AND created_at < ? ORDER BY created_at ASC", 
                                   (po['customer_email'], po['created_at'])).fetchall()
            opening_bal = sum((p['total_amount'] - (p.get('invoice_receipt', 0.0) or 0.0) - (p.get('amount_received', 0.0) or 0.0)) for p in all_pos)
            
            # Validation
            if amt_rec < 0:
                amt_rec = 0.0
            
            # Max payment allowed: previous debt + current invoice amount - already paid at delivery
            max_allowed = opening_bal + po['total_amount'] - (po.get('invoice_receipt', 0.0) or 0.0)
...
async def customer_cart(request: Request):
    ...
    return templates.TemplateResponse("customer_draft_review.html", {
        ...
        "all_prs_link": "/customer/all_prs?draft_id=0"
    })
...
async def view_draft_pr(request: Request, pr_id: int):
    ...
    return templates.TemplateResponse("customer_draft_review.html", {
        ...
        "recent_prs": recent_prs_list,
        "all_prs_link": f"/customer/all_prs?draft_id={pr_id}"
    })
...
@app.get("/customer/all_prs", response_class=HTMLResponse)
async def customer_all_prs(request: Request, draft_id: int = 0):
    user = require_customer(request)
    conn = get_db_connection()
    
    # Fetch all submitted PRs for the customer
    if DATABASE_URL:
        all_prs = conn.execute(
            "SELECT id, created_at, items_json FROM purchase_requisitions WHERE customer_email = %s AND status != 'DRAFT' ORDER BY created_at DESC",
            (user['email'],)
        ).fetchall()
    else:
        all_prs = conn.execute(
            "SELECT id, created_at, items_json FROM purchase_requisitions WHERE customer_email = ? AND status != 'DRAFT' ORDER BY created_at DESC",
            (user['email'],)
        ).fetchall()
    ...
    for pr in all_prs:
        try:
            ...
            all_prs_list.append({
                "id": pr['id'],
                "date": pr['created_at'].split()[0], # YYYY-MM-DD
                "count": len(items)
            })
        except: continue
    ...
    return templates.TemplateResponse("customer_all_prs.html", {
        "request": request,
        "all_prs": all_prs_list,
        "draft_id": draft_id
    })
...
async def submit_draft_pr(request: Request, pr_id: int):
    ...
    if DATABASE_URL:
        conn.execute("UPDATE purchase_requisitions SET status = 'PR', created_at = %s WHERE id = %s", (submitted_at, pr_id))
    else:
        conn.execute("UPDATE purchase_requisitions SET status = 'PR', created_at = ? WHERE id = ?", (submitted_at, pr_id))
...
async def view_quotation(request: Request, pr_id: int):
    ...
    if po and pr['status'] in ['ACCEPTED', 'PARTIALLY_ACCEPTED']:
        # Fetch dynamic delivery days from category
        delivery_days = []
        try:
            category_row = conn.execute("""
                SELECT cc.delivery_days 
                FROM customers c 
                JOIN customer_categories cc ON c.category = cc.name 
                WHERE c.email = ?
            """, (user['email'],)).fetchone()
            if category_row and category_row['delivery_days']:
                delivery_days = json.loads(category_row['delivery_days'])
                # Capitalize properly to match calculate_next_delivery_date ("Monday" etc)
                delivery_days = [d.capitalize() for d in delivery_days]
        except Exception as e:
            print(f"[DEBUG] Error fetching delivery days: {e}")

        # Calculate Expected Delivery Date
        from app.utils import calculate_next_delivery_date
        # Use order_placed_at or created_at (PO creation time)
        order_time_str = po.get('order_placed_at') or po.get('created_at') or datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        try:
            order_dt = datetime.strptime(order_time_str, "%Y-%m-%d %H:%M:%S")
        except:
            order_dt = datetime.now()
            
        expected_date_str = calculate_next_delivery_date(order_dt, delivery_days)
        expected_dt = datetime.strptime(expected_date_str, "%Y-%m-%d").date()
        today = datetime.now().date()
        
        # Determine Milestones
        is_invoiced = po.get('delivery_status') == 'INVOICED'
        ...
        delivery_data = {
            ...
            'stages': {
                ...
                'SCHEDULED': {
                    'label': 'Scheduled',
                    'sub_label': f"Expected: {expected_dt.strftime('%d %b (%a)')}",
                    'completed': True,
                    'active': not is_invoiced and today < expected_dt
                },
                'PACKED': {
                    'label': 'Packed',
                    'completed': is_invoiced or today >= (expected_dt - timedelta(days=1)),
                },
                'OUT_FOR_DELIVERY': {
                    'label': 'Out for Delivery',
                    'completed': is_invoiced or today >= expected_dt,
                },
                'DELIVERED': {
                    'label': 'Delivered',
                    'completed': is_invoiced,
...
async def customer_invoices(
    ...
    # Query: Get invoices for customer's POs (only FINAL invoices with invoice_no)
    query = '''
        SELECT 
            po.id,
            po.display_id,
...
async def customer_statement(
    ...
    try:
        ...
        # Get all POs for this customer, sorted OLD to NEW to calculate running balance
        query = '''
        ...
        # DEBUG: Return JSON to check if logic works and error is in template
        return JSONResponse({
            "ledger": ledger_entries,
            "date_from": date_from or "",
            "date_to": date_to or ""
...
async def admin_direct_sales_form(request: Request):
    ...
    # Get products with current SHOP stock
    inventory_service = InventoryService(conn)
    summary = inventory_service.get_stock_summary()
    # summary contains id, name, unit, shop_stock, etc.
    # Map it to match what the template expects plus shop_stock
    products = []
    for p in summary:
        # Also need 'rate' which isn't in get_stock_summary (oops)
        # Let's fetch rate explicitly
        rate_row = conn.execute("SELECT rate FROM products WHERE id = ?", (p['id'],)).fetchone()
        p['rate'] = rate_row['rate'] if rate_row else 0
        products.append(p)
...
async def admin_direct_sales_submit(
    ...
    # ===========================================================================
    # INVENTORY MANAGEMENT: Deduct stock for Direct Sale
    # ===========================================================================
    try:
        inventory_service = InventoryService(conn)
        for item in items:
            ...
            if product_id and qty_sold > 0:
                inventory_service.record_sale_out(
        ...
        print(f"[INFO] Stock deducted for Direct Sale {display_id}")
    except Exception as e:
        print(f"[WARN] Failed to reduce stock for Direct Sale {display_id}: {e}")
        # Not blocking as per requirement
...
async def create_invoice_form(request: Request, po_id: int):
    ...
    # Parse items for pre-filling and get SHOP stock
    inventory_service = InventoryService(conn)
    enriched_items = []
    try:
        raw_items = json.loads(po['items_json'])
        for item in raw_items:
            product_id = item.get('product_id')
            if product_id:
                item['shop_stock'] = inventory_service.get_product_shop_stock(product_id)
            else:
                item['shop_stock'] = 0
            enriched_items.append(item)
    except Exception as e:
        print(f"Error enriching items with stock: {e}")
        enriched_items = []
...
async def create_invoice_submit(request: Request, po_id: int):
    ...
    try:
        ...
        for i, item in enumerate(items):
            ...
            # Update item with delivered quantity
            updated_item = item.copy()
            updated_item['delivered_qty'] = delivered_qty
            updated_items.append(updated_item)
        
        # ============================================================================
        # INVENTORY MANAGEMENT: Managed by InventoryService (Logic below)
        # ============================================================================
        
        # Note: We skipped strict stock validation to allow negative stock (v1 requirement)
        # and to comply with "Allow Negative" rule. Errors will be logged but flow continues.
        
        # Step 2: Create invoice record (stock validation passed)
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, 'PENDING')
        ''', (po_id, invoice_no, created_at, payment_mode, delivery_remarks, json.dumps(updated_items)))
        
        invoice_id = cursor.lastrowid
        
        # Update PO delivery status and invoice receipt
        cursor.execute('''
            UPDATE purchase_orders 
            SET delivery_status = 'INVOICED', delivered_at = ?, items_json = ?, invoice_receipt = ?
            WHERE id = ?
        ''', (created_at, json.dumps(updated_items), amount_received, po_id))
        
        # Step 3: Deduct stock via InventoryService
        inventory_service = InventoryService(conn)
        
        for item in updated_items:
            product_id = item.get('product_id')
            qty_sold = item.get('delivered_qty', item.get('qty', 0))
            
            if not product_id or qty_sold <= 0:
                continue
                
            try:
                # Use 'record_sale_out' with commit=False to keep it in the same transaction
                inventory_service.record_sale_out(
                    product_id=product_id, 
                    qty=qty_sold, 
                ...
                print(f"[WARN] Failed to reduce stock for Product {product_id}: {e}")
                # We continue despite error to ensure Invoice is created (v1 requirement to not block Ops)
        
        print(f"[INFO] Stock deducted for {len([i for i in updated_items if i.get('product_id')])} items")
...
async def inventory_dashboard_page(request: Request):
    ...
    return templates.TemplateResponse("admin_inventory_dashboard.html", {
        "request": request,
        "user_email": user.get("email")
    })

@app.get("/admin/inventory/ledger/{product_id}", response_class=HTMLResponse)
async def inventory_ledger_page(product_id: int, request: Request):
    """Inventory ledger page for a specific product"""
    user = request.session.get("user")
